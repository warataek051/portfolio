...
export default function SensorDetailScreen() {
  ...
  const values = React.useMemo(
    () => sensorHistory.map((d) => d.value),
    [sensorHistory]
  );
  const dotColors = React.useMemo(
    () => sensorHistory.map((d) => getDotColor(d.status)),
    [sensorHistory]
  );
  ...
  return (
    <>
      {/* ✅ ตั้งค่า Header: ซ่อนชื่อ route และแสดงปุ่ม Back = "dashboard" บนทุกแพลตฟอร์ม */}
      <Stack.Screen
        options={{
          headerShown: true,
        
          headerTitle: () => null,     // ไม่แสดงข้อความตรงกลาง
      ...
      <View style={styles.container}>
        ...
        >
          ...
          {tooltip && (
            ...
            >
              ...
              {typeof tooltip.index === "number" && sensorHistory[tooltip.index]?.status && (
                <Text
                  style={[
                    styles.tooltipText,
        ...
        <View style={styles.statusRow}>
          <Text style={[styles.statusLabel, { color: "#43A047" }]}>ดี</Text>
          <Text style={[styles.statusLabel, { color: "#FFD600" }]}>ปานกลาง</Text>
          <Text style={[styles.statusLabel, { color: "#F44336" }]}>แย่</Text>
        </View>
...
function DetailContent({
  index, unit, point, prev, thresholds, color, onClose,
}: {
  index: number;
  unit: string;
  point: HistoryPoint;
  ...
  const statusLabel =
    point?.status === "good" ? "ดี" :
    point?.status === "warn" ? "ปานกลาง" :
    point?.status === "danger" ? "แย่" : "-";
    ...
    <View>
      ...
      <View style={styles.rowBetween}>
        ...
        <View style={{ alignItems: "flex-end" }}>
          ...
          <Text style={[styles.badge, { backgroundColor: soft(color), color }]}>{statusLabel}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  statusRow: { flexDirection: "row", justifyContent: "center", marginTop: 10, marginBottom: 6 },
  statusLabel: { fontWeight: "bold", fontSize: 15, marginHorizontal: 12 },
